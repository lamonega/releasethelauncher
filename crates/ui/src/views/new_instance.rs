...
fn process_messages(app: &mut App, state: &mut NewInstanceState) {
    ...
    for msg in messages {
        match msg {
            ...
            crate::UiMessage::ModrinthInstallResult(result) => match result {
                Ok(info) => handle_install_result(app, state, &info),
                Err(e) => {
...
fn handle_install_result(app: &mut App, state: &mut NewInstanceState, info: &str) {
...
fn show_modrinth_results(app: &App, ui: &mut egui::Ui, state: &mut NewInstanceState) {
    ...
    let installing_id = state.installing_modpack_id.clone();
    let status = state.modrinth_status.clone();
        ...
        .show(ui, |ui| {
            for result in &results {
                ui.group(|ui| {
                    ...
                    if installing_id == Some(result.id.clone()) {
                        ui.label(&status);
                    } else {
