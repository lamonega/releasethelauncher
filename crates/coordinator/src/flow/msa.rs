...
async fn run_msa_login(queue: &Queue) -> Result<release_the_launcher_auth::AccountData, String> {
    ...
    let code_resp = flow.request_device_code().await.map_err(|e| e.to_string())?;
    ...
    let msa_tokens = flow.poll_for_token(&code_resp).await.map_err(|e| e.to_string())?;
    let http = flow.http().clone();
    let client_id = flow.client_id().to_owned();

    let xbox_tokens = release_the_launcher_auth::xbox::get_xbox_tokens(&http, &msa_tokens.access_token)
        .await
...
pub async fn start_login(queue: Queue) {
    match run_msa_login(&queue).await {
        Ok(account) => push_event(&queue, Event::MsLoginSuccess { account: Box::new(account) }),
