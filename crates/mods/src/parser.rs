...
fn parse_fabric_mod_json(content: &str) -> Result<ModDetails, crate::ModsError> {
    ...
    let mod_id = json_str(&value, "id", "unknown");
    let name = json_str(&value, "name", &mod_id);
    let version = json_str(&value, "version", "unknown");
...
fn parse_mcmod_info(content: &str) -> Result<ModDetails, crate::ModsError> {
    ...
    let mod_id = json_str(&first, "modid", "unknown");
    let name = json_str(&first, "name", &mod_id);
    let version = json_str(&first, "version", "unknown");
    let description = json_str(&first, "description", "");
